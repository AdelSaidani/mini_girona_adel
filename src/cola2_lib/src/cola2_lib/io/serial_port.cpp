...
unsigned char SerialPort::readByte(const unsigned int msTimeout)
{
  ...
  else
  {
    ...
    timer.async_wait(boost::bind(&SerialPort::timedReadHandler, this, &timerResult, boost::placeholders::_1));

    boost::optional<boost::system::error_code> readResult;
    boost::asio::async_read(serialPort_, boost::asio::buffer(&byte, 1),
                            boost::bind(&SerialPort::timedReadHandler, this, &readResult, boost::placeholders::_1));
