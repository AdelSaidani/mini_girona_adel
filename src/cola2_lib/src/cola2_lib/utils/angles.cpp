...
double degreeMinutesToDegrees(const double degree_minutes, const char hemisphere)
{
  ...
  const double degrees = std::trunc(degree_minutes / 100.0);
  const double minutes = degree_minutes - (degrees * 100);
...
std::string degreesToDDMM_MMMMMM(const double degree)
{
  // Convert to ddmm.mmmmmm format (there should always be padding for the degrees and minutes)
  const double used_degrees = std::abs(degree);
  double degrees, minutes;
  std::stringstream stream;

  degrees = std::floor(used_degrees);
  minutes = (used_degrees - degrees) * 60.0;
  stream << std::setw(2) << std::setfill('0') << static_cast<int>(degrees);
  stream << std::setw(9) << std::setfill('0') << std::fixed << std::setprecision(6) << minutes;
