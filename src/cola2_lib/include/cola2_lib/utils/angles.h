...
/**
 * @brief Converts decimal degrees (DD.DD, double) to degree_minutes (DDMM.MMMMMM, string). Negative degrees values will
 * be converted to positive.
 *
 * @param degrees Decimal degrees (DD.DD). Negative values will be converted to positive.
 * @return Degrees and minutes, formatted as a string (DDMM.MMMMMM).
 */
std::string degreesToDDMM_MMMMMM(const double degrees);
