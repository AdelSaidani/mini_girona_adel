...
void IAUVROSController::initBase(const std::shared_ptr<rclcpp::Node> &node, std::shared_ptr<IAUVController> auv_controller_ptr, double period)
{
  ...
  // Publishers
  pub_wrench_ = node_->create_publisher<cola2_msgs::msg::BodyForceReq>(cola2::ros::getNamespace(node) + "/merged_body_force_req", 1);
  pub_merged_pose_ = node_->create_publisher<cola2_msgs::msg::WorldWaypointReq>(cola2::ros::getNamespace(node) + "/merged_world_waypoint_req", 1);
  pub_merged_twist_ = node_->create_publisher<cola2_msgs::msg::BodyVelocityReq>(cola2::ros::getNamespace(node) + "/merged_body_velocity_req", 1);
  pub_thrusters_setpoint_ = node_->create_publisher<std_msgs::msg::Float64MultiArray>(cola2::ros::getNamespace(node) + "/thruster_setpoints_sim", 1);

  // Subscribers --> WARNING! The buffer should be at least the size of maximum Request send per iteration/kind

  sub_nav_data_ = node_->create_subscription<cola2_msgs::msg::NavSts>("navigator/navigation", 2, std::bind(&IAUVROSController::updateNav, this, std::placeholders::_1));
  sub_ww_req_ = node_->create_subscription<cola2_msgs::msg::WorldWaypointReq>(cola2::ros::getNamespace(node) + "/world_waypoint_req", 10, std::bind(&IAUVROSController::updateWWR, this, std::placeholders::_1));
  sub_bv_req_ = node_->create_subscription<cola2_msgs::msg::BodyVelocityReq>(cola2::ros::getNamespace(node) + "/body_velocity_req", 10, std::bind(&IAUVROSController::updateBVR, this, std::placeholders::_1));
  sub_bf_req_ = node_->create_subscription<cola2_msgs::msg::BodyForceReq>(cola2::ros::getNamespace(node) + "/body_force_req", 10, std::bind(&IAUVROSController::updateBFR, this, std::placeholders::_1));

  _are_thrusters_killed = false;

  // Services
  enable_pose_controller_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/enable_pose_controller", std::bind(&IAUVROSController::enablePoseController, this, std::placeholders::_1, std::placeholders::_2));
  disable_pose_controller_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/disable_pose_controller", std::bind(&IAUVROSController::disablePoseController, this, std::placeholders::_1, std::placeholders::_2));
  enable_velocity_controller_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/enable_velocity_controller", std::bind(&IAUVROSController::enableVelocityController, this, std::placeholders::_1, std::placeholders::_2));
  disable_velocity_controller_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/disable_velocity_controller", std::bind(&IAUVROSController::disableVelocityController, this, std::placeholders::_1, std::placeholders::_2));
  enable_thruster_allocator_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/enable_thrusters", std::bind(&IAUVROSController::enableThrusterAllocator, this, std::placeholders::_1, std::placeholders::_2));
  disable_thruster_allocator_srv_ = node_->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(node) + "/disable_thrusters", std::bind(&IAUVROSController::disableThrusterAllocator, this, std::placeholders::_1, std::placeholders::_2));
...
void IAUVROSController::updateNav(const cola2_msgs::msg::NavSts::SharedPtr msg)
{
  ...
  last_nav_time_ = msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9; // CORRECT?
