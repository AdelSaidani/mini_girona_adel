...
void Pilot::init()
{
  ...
  srv_reload_params_ = this->create_service<std_srvs::srv::Trigger>(cola2::ros::getNamespace(shared_from_this()) + "/reload_params", reload_params_callback_func);

  // Publishers
  pub_wwr_ = this->create_publisher<cola2_msgs::msg::WorldWaypointReq>("controller/world_waypoint_req", 1);
  pub_bvr_ = this->create_publisher<cola2_msgs::msg::BodyVelocityReq>("controller/body_velocity_req", 1);

  pub_marker_ = this->create_publisher<visualization_msgs::msg::Marker>(cola2::ros::getNamespace(shared_from_this()) + "/waypoint_marker", 1);
  pub_goal_ = this->create_publisher<geometry_msgs::msg::PointStamped>(cola2::ros::getNamespace(shared_from_this()) + "/goal", 1);

  // Subscriber
  sub_nav_ = this->create_subscription<cola2_msgs::msg::NavSts>("navigator/navigation", 1, std::bind(&Pilot::navCallback, this, std::placeholders::_1));
  ...
  diagnostic_ = std::make_shared<cola2::ros::DiagnosticHelper>(shared_from_this(), "navigator/navigation", cola2::ros::getUnresolvedNodeName(shared_from_this()));
...
void Pilot::execute(const std::shared_ptr<GoalHandlePilot> goal_handle)
{
  ...
  while (rclcpp::ok())
  {
    ...
    cola2::utils::NED ned(current_state_.pose.position.ned_origin_latitude, current_state_.pose.position.ned_origin_longitude, 0.0);
    double dummy_depth;
    ned.geodetic2Ned(goal->initial_latitude, goal->initial_longitude, 0.0, request.initial_north, request.initial_east, dummy_depth);
    ned.geodetic2Ned(goal->final_latitude, goal->final_longitude, 0.0, request.final_north, request.final_east, dummy_depth);
...
void Pilot::publishControlCommands(const control::State &controller_output, const std::uint64_t priority, const rclcpp::Time &now)
{
  ...
  bvr.header.frame_id = std::string(this->get_namespace()) + "/base_link";
